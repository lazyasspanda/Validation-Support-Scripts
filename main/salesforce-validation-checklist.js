...
// @updateURL    https://raw.githubusercontent.com/DealerOnGirmada/office-tampermonkey-scripts/main/main/salesforce-validation-checklist.js
// @downloadURL  https://raw.githubusercontent.com/DealerOnGirmada/office-tampermonkey-scripts/main/main//salesforce-validation-checklist.js
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    try {
      GM_xmlhttpRequest({
        ...
        onload: function (response) {
          ...
          const releaseNotes = data.notes || "Improvements and bug fixes.";
          if (!latestVersion || latestVersion === CURRENT_VERSION) {
            console.log(
              `[Tampermonkey] Already up to date (v${CURRENT_VERSION}).`
            );
            return;
          }
          console.log(`[Tampermonkey] New version available: ${latestVersion}`);
          showUpdateBanner(latestVersion, releaseNotes);
        },
        onerror: function (err) {
          console.error("Network error while checking updates:", err);
        },
      });
    } catch (err) {
  ...
  function showUpdateBanner(newVersion, notes) {
    ...
    if (document.querySelector("#tm-update-banner")) return;
    const banner = document.createElement("div");
    banner.id = "tm-update-banner";
    banner.style.position = "fixed";
    banner.style.bottom = "20px";
    banner.style.right = "20px";
    banner.style.background = "#1e1e2f";
    ...
    banner.innerHTML = `
      <div style="font-size: 16px; font-weight: bold;">🚀 New version ${newVersion} available!</div>
      <div style="font-size: 13px; margin-top: 8px; color: #ddd;">${
        notes || "Improvements and bug fixes."
      }</div>
      <button id="tm-update-btn"
        style="
          background: rgb(251,116,28);
          color: white;
          border: none;
          border-radius: 6px;
          padding: 8px 14px;
    ...
    // Add event listeners
    document.querySelector("#tm-update-btn").addEventListener("click", () => {
      GM_openInTab(SCRIPT_URL, { active: true, insert: true });
      banner.remove();
    });
    document.querySelector("#tm-dismiss-btn").addEventListener("click", () => {
      banner.remove();
    });
  }
})();
